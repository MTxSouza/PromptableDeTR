...
class DeformableAttention(nn.Module):
    ...
    # Special methods.
    def __init__(self, in_embedding_dim, out_embedding_dim, num_points, patch_size, num_levels = 1):
        """
        ...
        Args:
            ...
            num_levels (int): The number of levels to deform the attention weights. (Default: 1)
        """
        super().__init__()

        # Layers.
        self.__offset = nn.Linear(in_features=in_embedding_dim, out_features=num_levels * num_points * 2, bias=False)
        self.__att_weight = nn.Linear(in_features=in_embedding_dim, out_features=num_levels * num_points, bias=False)
        self.__value = nn.Linear(in_features=in_embedding_dim, out_features=out_embedding_dim, bias=False)
    ...
    def __get_offsets_and_attention_weights(self, query):
        ...
        attention_weights = attention_weights.view(B, N, self.num_levels * self.num_points)
        attention_weights = F.softmax(input=attention_weights, dim=-1)
        attention_weights = attention_weights.view(B, N, self.num_levels, self.num_points)

        # Compute the offsets.
        offsets = self.__offset(query)
        offsets = offsets.view(B, N, self.num_levels, self.num_points, 2)
